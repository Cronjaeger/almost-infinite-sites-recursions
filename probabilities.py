...
def p_mutation(phi,i,j,k,theta,T,P):
    ...
    if phi.hasRow(new_row):
        ...
        return float (theta)/(sum(phi.n)-1+theta) * P[k,phi.S[i,j]] * (phi.L)/(T) * (n_r + 1)/(sum(phi.n))
    else:
        return float (theta)/(sum(phi.n)-1+theta) * P[k,phi.S[i,j]] * (phi.L)/(T) * (1)/(sum(phi.n))

def p_invisible(phi,i,k,theta,T,P):
    if (np.all(phi.S==0)):
        return float (theta)/(sum(phi.n)-1+theta) * P[k,0] * (1)/(sum(phi.n))
    else:
        return float (theta)/(sum(phi.n)-1+theta) * P[k,0] * (T - phi.L)/(T) * (1)/(sum(phi.n))
